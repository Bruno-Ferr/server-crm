...
export const getScheduleArray = async (date) => {
  const scheduleArray = await db("agendamentos as a")
    .select('a.id AS agendamento_id', 'a.data AS agendamento_data', 'clt.nome AS agendamento_cliente_nome', 'clt.cpf AS agendamento_cliente', 
    'vclo.placa AS agendamento_veiculo_placa', 'vclo.modelo AS agendamento_veiculo_modelo', 'vclo.id AS agendamento_veiculo', 'srvc.tipo AS agendamento_servico', 
    'a.status AS agendamento_notificacao', 'o.*', 'c.*')
